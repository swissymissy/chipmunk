...
func (cfg *ApiConfig) HandlerAddStudentToPastSessionRecord(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		log.Printf("error converting seesion id to int: %s\n", err)
	...
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			...
			ResponseWithError(w, http.StatusBadRequest, "unable to add to student to session. Student isn't enrolled in this course or already in this session")
			return
		}
		log.Printf("error adding student to this session: %s\n", err)
		ResponseWithError(w, http.StatusInternalServerError, "unable to add studen to this session.")
		return
	}

	// response
	ResponseWithJSON(w, 201, AddStudentToPastSessionRecordRes{
